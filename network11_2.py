...
class net(nn.Module):
    def __init__(self, image_size=32, patch_size=4, dim=512, heads=8, mlp_dim=512, budget=10, desired=10, channels = 3, dropout = 0.05, emb_dropout = 0.05):
        super().__init__()
    ...
    def forward(self, x, y):
        ...
        if self.training:
            x, halt = self.module1(x) # (B, max, class), (B, max)
            ...
            loss = ce_loss.mean() + halt_ce_loss.mean() + (1/self.desired) * abs_halt.mean()
        
            return loss
        
        else:
            x, cnt = self.module1(x) # (B, class), N
            return x, cnt
    ...
    def forward(self, x):
        ...
        if self.training:
            for i in range(self.max):
                ...
                output = self.classifier[i](x[:,0]) # (B, 65, 512) -> (B, 512) -> (B, 10)
                
                halt = self.halt[i](x.mean(dim=1)).squeeze(1)
...
if __name__ == "__main__":
    ...
    b.eval()
    a = torch.randn((1,3,32,32)).to(device).softmax(dim=-1)
