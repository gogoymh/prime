...
class net(nn.Module):
    def __init__(self, image_size=32, patch_size=4, dim=512, heads=8, mlp_dim=512, budget=50, channels = 3, dropout = 0.05, emb_dropout = 0.05):
        super().__init__()
    ...
    def forward(self, x, y):
        ...
        if self.training:
            ...
            #delta = (1/ce_loss.min(1)[0]).softmax(dim=-1).detach()
            ...
            #loss = ce_loss.mean() + halt_ce_loss.mean() + (1/self.max) * abs_halt.mean()
            loss = ce_loss.mean() + halt_ce_loss.mean() + (1/10) * abs_halt.mean()
